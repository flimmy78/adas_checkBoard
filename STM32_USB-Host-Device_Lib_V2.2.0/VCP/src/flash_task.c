...
static void flash_send_write_first_resp_msg(void)
{
	...
	/* Add file packet tlv */
	file_packet = (TP_FILE_PACKET_t *)&msg_text->buff[UART_HDR_LENGTH+TLV_FILE_PARA_LENGTH];
	...
	crc32 = GetCrc32((u8 *)((u32)uart_head + CRC_REMOVE_LENGTH),uart_head->msg_len - CRC_REMOVE_LENGTH);
...
static void flash_send_write_resp_msg(void)
{
	...
	/*1. Malloc memory */
	//malloc_size = CALC_MALLOC_SIZE(sizeof(hi3518_UART_msg_t)+sizeof(msg_head_t)+ UART_HDR_LENGTH +TLV_FILE_PARA_LENGTH+TP_FILE_PACKET_LENGTH);
	malloc_size = CALC_MALLOC_SIZE(sizeof(hi3518_UART_msg_t)+sizeof(msg_head_t)+ UART_HDR_LENGTH );
...
static void flash_write_firmware(msg_head_t * msg_head)
{
	...
	if (net_head->msg_type == MSG_FILE_WRITE_FIRST_REQ)
	{
		...
		file_packet	= (TP_FILE_PACKET_t *)((u32)net_head + UART_HDR_LENGTH+TLV_FILE_PARA_LENGTH);
		if((file_para->type ==TP_FILE_PARA_ID)&&(file_packet->type == TP_FILE_PACKET_ID))
		{
			flash_file.curpos			= 0;
			...
			memcpy((u8 *)&flash_file.filename,(u8 *)&file_para->filename,MAX_FILENAME_LEN);
		}
		...
		pbuff = (u8 *)((u32)net_head + TLV_FILE_PARA_LENGTH + UART_HDR_LENGTH+TP_FILE_PACKET_LENGTH);
		...
		NEW_SPI_FLASH_BufferWrite(pbuff, flash_file.curpos+flash_start_addr, file_packet->cur_data_length);
	...
	else if (net_head->msg_type == MSG_FILE_WRITE_REQ)
	{
		...
		if((flash_file.file_flag !=NULL)&&(file_packet->type == TP_FILE_PACKET_ID))
		{
	...
	if(file_packet->cur_packet == flash_file.total_packets)
	{
		...
		{
			if(flash_file.file_flag	 == MCU_SF_FILE_NUM || flash_file.file_flag	 == FPGA_SF_FILE_NUM)
			{
				//update flag
				if (flash_update_firmware_info()) flash_update_firmware_reply(true);
				else flash_update_firmware_reply(false);
			...
			{
				cur_file.filelen = 0;
				if (flash_check_file_crc32(PICTURE_ADDR,flash_file.filelen,flash_file.crc32))
				{
				...
				else TraceStr("update picture crc32 error \r\n");
			}
...
static void flash_send_read_first_resp_msg(TLV_FILE_PARA_t * read_file_para)
{
	...
	/*1. Malloc memory */
	malloc_size = CALC_MALLOC_SIZE(sizeof(hi3518_UART_msg_t)+sizeof(msg_head_t)+ UART_HDR_LENGTH + TLV_FILE_PARA_LENGTH + TP_FILE_PACKET_LENGTH + read_len);
	msg_addr = pvPortMalloc(malloc_size);
	 ...
	 
	{
		...
		filelen =  read_len;
		msg_text->buff = (u8 *)((u32)msg_text + sizeof(hi3518_UART_msg_t));
		msg_text->len = UART_HDR_LENGTH +TLV_FILE_PARA_LENGTH +TP_FILE_PACKET_LENGTH+filelen;
...
//	msg_queue_t 		msg_queue;
	...
	else if((uart_head->msg_type== MSG_FILE_WRITE_FIRST_REQ) ||(uart_head->msg_type==MSG_FILE_WRITE_REQ))
	{
...
static bool flash_update_firmware_info(void)
{
	...
	if (flash_check_file_crc32(firm_info->firm_start,firm_info->firm_length,firm_info->firm_crc32))
	{
		/*Update crc32 */
...
static void flash_update_firmware_reply(bool success)
{
	...
	/*1. Malloc memory */
	malloc_size = CALC_MALLOC_SIZE(sizeof(hi3518_UART_msg_t)+sizeof(msg_head_t)+ UART_HDR_LENGTH + TLV_FILE_PARA_LENGTH);
	msg_addr = pvPortMalloc(malloc_size);
	if (msg_addr == NULL)
	{
		return;
	...
	crc32 = GetCrc32((u8 *)((u32)net_head + CRC_REMOVE_LENGTH),net_head->msg_len - CRC_REMOVE_LENGTH);
