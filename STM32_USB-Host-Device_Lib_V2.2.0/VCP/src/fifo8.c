...
{
	u16 tmplen;
	if ( (stFIFO->FIFO_size-FIFOLen(stFIFO)) <= len)		return false;
	//one copy
	if (stFIFO->head > stFIFO->rear)
	{
...
static bool RcvFrameFromFIFO(FIFO_t *stFIFO, u8 *framebuf)
{
	...
	while (1)
	{
		if (FIFOLen(stFIFO)<(UART_HDR_LENGTH)) return false;  /* Frame not receive finished, return */
		if (CheckFIFOByte(stFIFO)==(UART_START_BYTES&0xFF))
		{
				...
				if ((frame_len>=MAX_HI3518_UART_FRAME_BUFFER_SIZE) || (frame_len<UART_HDR_LENGTH)) /* Something error,but head is matched */
				{
