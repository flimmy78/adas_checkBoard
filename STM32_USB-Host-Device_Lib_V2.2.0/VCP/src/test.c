...
static bool WriteDeviceFileReply(u8 MSG_TYPE , comm_head_t *msg_head,bool status);
/****************************************************************************/
		...
		switch(msg_type)
		{
			...
			case MSG_TYPE_WRITE_DEVICE_FILE_REQ:
				...
				if(i == 1000)
				{
					...
					WriteDeviceFileReply(MSG_TYPE_WRITE_DEVICE_FILE_RESP , msg_head , false);//return fail
				}
				else
					WriteDeviceFileReply(MSG_TYPE_WRITE_DEVICE_FILE_RESP , msg_head , true);//return ok
				break;
			...
			case MSG_TYPE_WRITE_DEVICE_FILE_MORE_REQ:
				...
				{
					TraceStr("\r\nNO ACK FROM MAINBOARD");
					WriteDeviceFileReply(MSG_TYPE_WRITE_DEVICE_FILE_MORE_RESP , msg_head,false);//return faile
				}
				else
					WriteDeviceFileReply(MSG_TYPE_WRITE_DEVICE_FILE_MORE_RESP , msg_head,true);//return ok
				break;
...
static bool writeFileToMCUack()
{
	...
	if (UartRxFifo.rear >= UartRxFifo.head)
	{
		...
		{
			if((UartRxFifo.databuf[UartRxFifo.head] == 0xcc) && (UartRxFifo.databuf[UartRxFifo.head + 1] == 0xcc)&& (UartRxFifo.databuf[UartRxFifo.head + 2] == 0xcc)&& (UartRxFifo.databuf[UartRxFifo.head + 3] == 0xcc))
			{
				if((UartRxFifo.databuf[UartRxFifo.head + 0x0a] == 0x01) && (UartRxFifo.databuf[UartRxFifo.head + 0x0b] == 0x20))
				{
					UartRxFifo.head = (UartRxFifo.head + UartRxFifo.databuf[UartRxFifo.head + 8] + UartRxFifo.databuf[UartRxFifo.head + 9] * 256) % MAX_COMM_UART_DMA_RCV_SIZE;
					return  true;
				}
				else if((UartRxFifo.databuf[UartRxFifo.head + 0x0a] == 0x01) && (UartRxFifo.databuf[UartRxFifo.head + 0x0b] == 0x21))
				{
					UartRxFifo.head = (UartRxFifo.head + UartRxFifo.databuf[UartRxFifo.head + 8] + UartRxFifo.databuf[UartRxFifo.head + 9] * 256) % MAX_COMM_UART_DMA_RCV_SIZE;
					return  true;
				}
				else if((UartRxFifo.databuf[UartRxFifo.head + 0x0a] == 0x01) && (UartRxFifo.databuf[UartRxFifo.head + 0x0b] == 0x22))
				{
					UartRxFifo.head = (UartRxFifo.head + UartRxFifo.databuf[UartRxFifo.head + 8] + UartRxFifo.databuf[UartRxFifo.head + 9] * 256) % MAX_COMM_UART_DMA_RCV_SIZE;
					send_flag = 0;
					return  true;
	...
	{
		while((MAX_COMM_UART_DMA_RCV_SIZE + UartRxFifo.rear - UartRxFifo.head) >= 16 + 10)
		{
			if((UartRxFifo.databuf[(MAX_COMM_UART_DMA_RCV_SIZE + UartRxFifo.head) % MAX_COMM_UART_DMA_RCV_SIZE] == 0xcc) && (UartRxFifo.databuf[(MAX_COMM_UART_DMA_RCV_SIZE + UartRxFifo.head + 1) % MAX_COMM_UART_DMA_RCV_SIZE] == 0xcc)&& (UartRxFifo.databuf[(MAX_COMM_UART_DMA_RCV_SIZE + UartRxFifo.head + 2) % MAX_COMM_UART_DMA_RCV_SIZE] == 0xcc)&& (UartRxFifo.databuf[(MAX_COMM_UART_DMA_RCV_SIZE + UartRxFifo.head + 3) % MAX_COMM_UART_DMA_RCV_SIZE ] == 0xcc))
			{
				if((UartRxFifo.databuf[(MAX_COMM_UART_DMA_RCV_SIZE + UartRxFifo.head + 0x0a) % MAX_COMM_UART_DMA_RCV_SIZE] == 0x01) && (UartRxFifo.databuf[(MAX_COMM_UART_DMA_RCV_SIZE + UartRxFifo.head + 0x0b)] == 0x20))
				{
					UartRxFifo.head = (UartRxFifo.head + UartRxFifo.databuf[(MAX_COMM_UART_DMA_RCV_SIZE + UartRxFifo.head + 0x08) % MAX_COMM_UART_DMA_RCV_SIZE] + UartRxFifo.databuf[(MAX_COMM_UART_DMA_RCV_SIZE + UartRxFifo.head + 0x09) % MAX_COMM_UART_DMA_RCV_SIZE] * 256) % MAX_COMM_UART_DMA_RCV_SIZE;
					return true;
				}
				else if((UartRxFifo.databuf[(MAX_COMM_UART_DMA_RCV_SIZE + UartRxFifo.head + 0x0a) % MAX_COMM_UART_DMA_RCV_SIZE] == 0x01) && (UartRxFifo.databuf[(MAX_COMM_UART_DMA_RCV_SIZE + UartRxFifo.head + 0x0b)] == 0x21))
				{
					UartRxFifo.head = (UartRxFifo.head + UartRxFifo.databuf[(MAX_COMM_UART_DMA_RCV_SIZE + UartRxFifo.head + 0x08) % MAX_COMM_UART_DMA_RCV_SIZE] + UartRxFifo.databuf[(MAX_COMM_UART_DMA_RCV_SIZE + UartRxFifo.head + 0x09) % MAX_COMM_UART_DMA_RCV_SIZE] * 256) % MAX_COMM_UART_DMA_RCV_SIZE;
					return true;
				}
				else if((UartRxFifo.databuf[(MAX_COMM_UART_DMA_RCV_SIZE + UartRxFifo.head + 0x0a) % MAX_COMM_UART_DMA_RCV_SIZE] == 0x01) && (UartRxFifo.databuf[(MAX_COMM_UART_DMA_RCV_SIZE + UartRxFifo.head + 0x0b)] == 0x22))
				{
					UartRxFifo.head = (UartRxFifo.head + UartRxFifo.databuf[(MAX_COMM_UART_DMA_RCV_SIZE + UartRxFifo.head + 0x08) % MAX_COMM_UART_DMA_RCV_SIZE] + UartRxFifo.databuf[(MAX_COMM_UART_DMA_RCV_SIZE + UartRxFifo.head + 0x09) % MAX_COMM_UART_DMA_RCV_SIZE] * 256) % MAX_COMM_UART_DMA_RCV_SIZE;
					send_flag = 0;
					return true;
...
typedef struct{
  ...
  unsigned	char	Rssi;               /* The computed RSSI value in db of current frame */
} WIFI_TEST_RESAULT;
static WIFI_TEST_RESAULT wifi_test_resault;

static void TestServiceCmd(comm_head_t *msg_head)
{
	unsigned char *temp;unsigned int i = 0;unsigned short current_Pack_Length = 0;unsigned int pack_nNum_Now = 0;
	unsigned int sum;
		...
		switch(msg_type)
		{
			...
			/*
				...
				*/
				...
				baseaddr = temp[16+4] + temp[16+5]*256 +temp[16+6]*256*256 + temp[16+7]*256*256*256;
				for(i = 0; (i< 4)&(current_Pack_Length > 0);)
				{
					...
					{
						if (FirmCmdWrite(&temp[16+8 + i*256],baseaddr,current_Pack_Length)==false)
						{
							TraceStr("CMD_TEST_MCU_CODE WRITE FAILED\r\n");
							if(CommBuf[0] == 0x1f)
								TestCmdReply(true,MSG_TYPE_CMD_TEST_MCU_CODE_RESP,ACK_RUN_NACK);
			...
			case MSG_TYPE_CMD_TEST_WIFI_TEST_REQ:
				...
				//send start test cmd
				wifi_start_cmd.head[0] = wifi_start_cmd.head[1] = wifi_start_cmd.head[2] = wifi_start_cmd.head[3] = 0xcc;
				wifi_start_cmd.head[4] = wifi_start_cmd.head[5] = wifi_start_cmd.head[6] = wifi_start_cmd.head[7] = 0x00;
				wifi_start_cmd.head[8] = 0x10;
				wifi_start_cmd.head[9] = wifi_start_cmd.head[10] = wifi_start_cmd.head[11] = wifi_start_cmd.head[12] = wifi_start_cmd.head[13] = wifi_start_cmd.head[14] = wifi_start_cmd.head[15] = 0x00;
...
static void TestReplyFunc(void)
{
	...
	for(i = 0; i < 10 ; i++)
	{
		...
		else
		{
			if((UartRxFifo.databuf[(MAX_COMM_UART_DMA_RCV_SIZE + UartRxFifo.head) % MAX_COMM_UART_DMA_RCV_SIZE] == 0xcc) && (UartRxFifo.databuf[(MAX_COMM_UART_DMA_RCV_SIZE + UartRxFifo.head + 1) % MAX_COMM_UART_DMA_RCV_SIZE] == 0xcc)&& (UartRxFifo.databuf[(MAX_COMM_UART_DMA_RCV_SIZE + UartRxFifo.head + 2) % MAX_COMM_UART_DMA_RCV_SIZE] == 0xcc)&& (UartRxFifo.databuf[(MAX_COMM_UART_DMA_RCV_SIZE + UartRxFifo.head + 3) % MAX_COMM_UART_DMA_RCV_SIZE ] == 0xcc))
			{
				if((UartRxFifo.databuf[(MAX_COMM_UART_DMA_RCV_SIZE + UartRxFifo.head + 8) % MAX_COMM_UART_DMA_RCV_SIZE] + UartRxFifo.databuf[(MAX_COMM_UART_DMA_RCV_SIZE + UartRxFifo.head + 9) % MAX_COMM_UART_DMA_RCV_SIZE ] * 256) == 16 + 5)
				{
					if(UartRxFifo.databuf[(MAX_COMM_UART_DMA_RCV_SIZE + UartRxFifo.head + 16 + 4) % MAX_COMM_UART_DMA_RCV_SIZE] == OP_CAN_Test_fail)
					{
...
u16 RcvWIFI_FirmFrameFromUart(FIFO_t *stFIFO, u8 *framebuf,u8 length)
{
	...
	//TraceStr("RcvFirmFrameFromUart\r\n");
	len = FIFOLen(stFIFO);
	if (len < length ) return 0;  /* Frame not receive finished, return */
...
static bool uart_DMA_finished(void)
{
	...
	if ((DMA_GetFlagStatus(DMA2_FLAG_TC5)==SET)  || (DMA_GetCurrDataCounter(COMM_UART_TX_DMA)==0))
	{
...
static bool WifiTest_uart_DMA_finished(void)
{
	...
	if ((DMA_GetFlagStatus(DMA1_FLAG_TC7)==SET)  || (DMA_GetCurrDataCounter(WifiTest_UART_TX_DMA)==0))
	{
...
static bool RcvFrameFromFIFO(FIFO_t *stFIFO, u8 *framebuf)
{
	...
	if(stFIFO->rear < stFIFO->head)
	{
		...
		{
			if((stFIFO->databuf[stFIFO->head] == 0xcc) && (stFIFO->databuf[(stFIFO->head + 1) % MAX_COMM_UART_DMA_RCV_SIZE] == 0xcc) && (stFIFO->databuf[(stFIFO->head + 2) % MAX_COMM_UART_DMA_RCV_SIZE] == 0xcc) && (stFIFO->databuf[(stFIFO->head + 3) % MAX_COMM_UART_DMA_RCV_SIZE] == 0xcc))
			{
				return_len = stFIFO->databuf[(stFIFO->head + 8)% MAX_COMM_UART_DMA_RCV_SIZE] + stFIFO->databuf[(stFIFO->head + 9) % MAX_COMM_UART_DMA_RCV_SIZE]*256;
	...
	else
	{
		...
		{
			if((stFIFO->databuf[stFIFO->head] == 0xcc) && (stFIFO->databuf[stFIFO->head + 1] == 0xcc) && (stFIFO->databuf[stFIFO->head + 2] == 0xcc) && (stFIFO->databuf[stFIFO->head + 3] == 0xcc))
			{
				return_len = stFIFO->databuf[stFIFO->head + 8] + stFIFO->databuf[stFIFO->head + 9]*256;
...
bool checkForTestACK(u8 OP_CODE)
{
	...
	if (UartRxFifo.rear >= UartRxFifo.head)
	{
		while((UartRxFifo.rear - UartRxFifo.head) >= 16 + 5)
		{
			if((UartRxFifo.databuf[UartRxFifo.head] == 0xcc) && (UartRxFifo.databuf[UartRxFifo.head + 1] == 0xcc)&& (UartRxFifo.databuf[UartRxFifo.head + 2] == 0xcc)&& (UartRxFifo.databuf[UartRxFifo.head + 3] == 0xcc))
			{
				if((UartRxFifo.databuf[UartRxFifo.head + 8] + UartRxFifo.databuf[UartRxFifo.head + 9] * 256) == 16 + 5)
				{
	...
	{
		while((MAX_COMM_UART_DMA_RCV_SIZE + UartRxFifo.rear - UartRxFifo.head) >= 16 + 5)
		{
			if((UartRxFifo.databuf[(MAX_COMM_UART_DMA_RCV_SIZE + UartRxFifo.head) % MAX_COMM_UART_DMA_RCV_SIZE] == 0xcc) && (UartRxFifo.databuf[(MAX_COMM_UART_DMA_RCV_SIZE + UartRxFifo.head + 1) % MAX_COMM_UART_DMA_RCV_SIZE] == 0xcc)&& (UartRxFifo.databuf[(MAX_COMM_UART_DMA_RCV_SIZE + UartRxFifo.head + 2) % MAX_COMM_UART_DMA_RCV_SIZE] == 0xcc)&& (UartRxFifo.databuf[(MAX_COMM_UART_DMA_RCV_SIZE + UartRxFifo.head + 3) % MAX_COMM_UART_DMA_RCV_SIZE ] == 0xcc))
			{
				if((UartRxFifo.databuf[(MAX_COMM_UART_DMA_RCV_SIZE + UartRxFifo.head + 8) % MAX_COMM_UART_DMA_RCV_SIZE] + UartRxFifo.databuf[(MAX_COMM_UART_DMA_RCV_SIZE + UartRxFifo.head + 9) % MAX_COMM_UART_DMA_RCV_SIZE ] * 256) == 16 + 5)
				{
					if(UartRxFifo.databuf[(MAX_COMM_UART_DMA_RCV_SIZE + UartRxFifo.head + 16 + 4) % MAX_COMM_UART_DMA_RCV_SIZE] == OP_CODE)
						return true;
					else
						UartRxFifo.head = (UartRxFifo.head + 1) % MAX_COMM_UART_DMA_RCV_SIZE;
				}
...
/***********************************  add by yuzongyong  **************************************/
static unsigned char commBuf[1024];   static int rcvFlag = 0; static int rcvLen = 0;
...
bool waitForACK(int times,u8 followLen)
{
	...
	for(i = 0;i<times;i++)
	{
		...
		if(len >= 2)
		{
			...
			if((commBuf[len - followLen - 2] == 0x00)&&(commBuf[len - followLen - 1] == 0xcc))
				break;
			else if((commBuf[len - followLen - 2] == 0x00)&&(commBuf[len - followLen - 1] == 0x33))
			{
				TraceStr("\r\n NACK");
			}
			else
...
bool WIFI_Product_Restart()
{
	...
	TraceStr("\r\nRestart WIFI device");
	RCC_APB2PeriphClockCmd(RCC_APB2Periph_GPIOE, ENABLE); 
	GPIO_InitStructure.GPIO_Pin = wifi_ctl_power_PIN | wifi_ctl_Product_sop1_PIN | wifi_ctl_Product_RST_PIN;
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_PP;
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
...
bool WIFI_TestBoard_Restart()
{
	...
	RCC_APB2PeriphClockCmd(RCC_APB2Periph_GPIOE, ENABLE); 
	GPIO_InitStructure.GPIO_Pin = wifi_ctl_power_PIN | wifi_ctl_TestBoard_sop1_PIN | wifi_ctl_TestBoard_RST_PIN;
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_PP;
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
	GPIO_Init(wifi_ctl_PORT, &GPIO_InitStructure);
...
bool FS_Programming_TestBoard(u8 *msg_data)
{
	...
	RCC_APB2PeriphClockCmd(RCC_APB2Periph_GPIOE, ENABLE); 
	//RST_WIFI_PIN
	GPIO_InitStructure.GPIO_Pin = wifi_ctl_power_PIN | wifi_ctl_TestBoard_RST_PIN | wifi_ctl_TestBoard_sop1_PIN;
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_PP;
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
	GPIO_Init(GPIOE, &GPIO_InitStructure);
	if(msg_data[7] == 0x01)
...
void download_WIFI_Write_Sram_Patch(u8 *msg_data)
{
	if((msg_data[16+2] + msg_data[16+3]*256) == 0)  
	{
		...
		if(Execute_from_RAM())
		{
			...
			//reply to pc
			TestCmdReply(true,MSG_TYPE_CMD_TEST_WIFI_CODE_WRITE_SRAM_PATCH_RESP,ACK_RUN_OK);
		}
		else
		{
			TraceStr("\r\nDownload WIFI : Execute from RAM failed\r\n");
			TestCmdReply(true,MSG_TYPE_CMD_TEST_WIFI_CODE_WRITE_SRAM_PATCH_RESP,ACK_RUN_ERROR);
		}
	}
	else
	{
...
void download_WIFI_Write_Flash_Patch(u8 *msg_head)
{
	...
	{
		//reply to pc
		TestCmdReply(true,MSG_TYPE_CMD_TEST_WIFI_CODE_WRITE_FLASH_PATCH_RESP,ACK_RUN_OK);
	}
	else
	{
		TraceStr("\r\nDownload WIFI : Raw Storage Write FLASH patch failed\r\n");
		TestCmdReply(true,MSG_TYPE_CMD_TEST_WIFI_CODE_WRITE_FLASH_PATCH_RESP,ACK_RUN_ERROR);
	}
}
