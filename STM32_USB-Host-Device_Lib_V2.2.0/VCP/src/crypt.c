...
unsigned char
cpukey[12];// = {0x33,0xff,0xd8,0x05,0x47,0x4b,0x39,0x30,0x47,0x30,0x18,0x43};
...

{
	...
	if (i == 0) {
		CryptInLen = code_scatter.code_len;
	} else {
		CryptInLen = (code_scatter.code_len + CRYPT1_IN_LEN) / CRYPT1_IN_LEN *
		             CRYPT1_IN_LEN;
	}
	...
	memcpy(&CryptBinBuf[CRYPT_MORE_DATA_ADDR], &CryptOutBuf[code_scatter.code_len],
	       CryptOutLen - code_scatter.code_len);
...
void decrypt_main(void)
{
	...
	i = code_scatter.code_len % CRYPT1_IN_LEN;
	if (i == 0) {
		i = code_scatter.code_len;
	} else {
		i = (code_scatter.code_len + CRYPT1_IN_LEN) / CRYPT1_IN_LEN * CRYPT1_IN_LEN;
	}
	...
	memcpy(&DeCryptInBuf[code_scatter.code_len], &BinBuf[CRYPT_MORE_DATA_ADDR],
	       DeCryptInLen - code_scatter.code_len);
...
int _tmain(int argc, _TCHAR *argv[])
{
...
#ifdef DEBUG_CODE
	sprintf_s(mapfile, MAX_FILENAME_LEN, "%s",
	          "F://work//mcu_m4//obj//mcu_code.map");
	printf("filename = %s\r\n", mapfile);
	...
	sprintf_s(cryptfile, MAX_FILENAME_LEN, "%s",
	          "F://work//mcu_m4//crypt_code.bin");
	printf("filename = %s\r\n", binfile);
#else
	if (argc < 5) {
		help();
	...
	//	if (argc == 5)     // CPU key is must
	{
		...
		sscanf_s(tmpbuf, "%2X%2X%2X%2X%2X%2X%2X%2X%2X%2X%2X%2X", &cpukey[0], &cpukey[1],
		         &cpukey[2], &cpukey[3], &cpukey[4], &cpukey[5], &cpukey[6], &cpukey[7],
		         &cpukey[8], &cpukey[9], &cpukey[10], &cpukey[11]);
		printf("cpukey[12] = 0x%02x,0x%02x,0x%02x,0x%02x,0x%02x,0x%02x,0x%02x,0x%02x,0x%02x,0x%02x,0x%02x,0x%02x",
		       cpukey[0], cpukey[1], cpukey[2], cpukey[3], cpukey[4], cpukey[5], cpukey[6],
		       cpukey[7], cpukey[8], cpukey[9], cpukey[10], cpukey[11]);
		random_mode = 1;

		if (tmpbuf == "33FFD805474B393047301843") {
			printf("Now is fixed cpu key mode \r\n");
		}
...
static signed short   encrypt_41F230(char a1, char a2, const void *input_buffer,
                                     u8 *a4)   //a4 = xx8700 +62H, output buffer, input_buffer is input buffer,v44, 0x1c length, a2 length = 0x21,  a1 = v34 or -2
{
...
//1307
static signed short   decrypt_41F2C0(u8 *input_buffer,
                                     u8 *a2)   // input_buffer input, a2 output, decrypt   this function test ok
{
	u8 v2; // ecx@1
	u8 v3; // bl@1
	u8 *v4; // esi@1
