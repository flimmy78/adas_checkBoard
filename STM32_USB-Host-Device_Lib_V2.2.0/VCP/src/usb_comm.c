...
bool RcvFrameFromUsb(FIFO_t *stFIFO, u8 *framebuf)
{
	...
	while (1) {
		...
		if (CheckFIFOByte(stFIFO) == (USB_HEAD_START_BYTES & 0xFF)) {
			...
			msg_head = (comm_head_t *)framebuf;
			if ((msg_head->service_type == 0x01) && ((msg_head->msg_type == 0x20)
			        || (msg_head->msg_type == 0x21))) {
				temp = (u8 *)msg_head;
			...
			if ((frame_len >= MAX_USB_FRAME_LENGTH)
			    || (frame_len <
			        MIN_USB_FRAME_LENGTH)) { /* Something error,but head is matched */
...
static void MsgSendFromUsb(comm_head_t *usb_msg_head, task_id_enum totask)
{
	...
	/*1. Malloc memory */
	if ((usb_msg_head->service_type == 0x01) && ((usb_msg_head->msg_type == 0x20)
	        || (usb_msg_head->msg_type == 0x21))) {
		len = tmp[8] + tmp[9] * 256;
